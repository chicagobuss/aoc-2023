...
for digit_pair in digits:
    digit_name = digit_pair['n']
    digit = digit_pair['v']
    input = input.replace(digit_name, digit)

line_count = 0
all_digits = []

og_split = og_input.split('\n')
...
for line in input.split('\n'):
    ...
    print("appended %s for line %s, originally %s" % (digit, line, og_split[line_count]))
    line_count += 1

final = sum(all_digits)
print("got %s for the final" % final)