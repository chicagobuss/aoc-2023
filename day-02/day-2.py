...
input = """Game 1: 18 red, 8 green, 7 blue; 15 red, 4 blue, 1 green; 2 green, 17 red, 6 blue; 5 green, 1 blue, 11 red; 18 red, 1 green, 14 blue; 8 blue
...
for l in input.split('\n'):
  i = int(l.split(' ')[1].split(':')[0])
  is_possible = True
  tries = l.split(':')[1]
  max_seen = {'red': 0, 'green': 0, 'blue': 0}
  for t in tries.split(';'):
    for color in colors:
       if color in t:
         c_num = int(t.split(color)[0].split(',')[-1].strip())
         if c_num > max_seen[color]:
           max_seen[color] = c_num
         color_max = colors_max[color]
         if c_num > color_max:
            print('game with index %s was impossible due to color %s number %s being higher than color_max %s in try: %s' % (i, color, c_num, color_max, t))
            is_possible = False
  power = 1
  for color in colors:
    power *= max_seen[color]
  game_powers[i] = power
  game_powers_array.append(power)
  if is_possible:
    possible_games.append(i)   
print("All possible games: %s" % possible_games)
print(sum(possible_games))
print("Sum of all powers: %s" % sum(game_powers_array))